...
try:
	...
	if not "Signed" in sign_status.decode("UTF-8"):
		print("[E] APK signing error %s" % (sign_status))

	
	sign_verify = subprocess.check_output(["apksigner", "verify", "--verbose", signed_apk_path])

	if not "Verified using v1 scheme (JAR signing): true" in sign_verify.decode("UTF-8") and not "Verified using v2 scheme (APK Signature Scheme v2): true" in sign_verify.decode("UTF-8"):
		print(sign_verify)
	else:
